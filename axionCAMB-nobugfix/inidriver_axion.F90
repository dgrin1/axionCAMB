...
! End axion stuff
! Command line read to interface with python
...
!Conditional to activate getting cmb scalar cls
	if (int1.eq.1) then
...
!MAKE lmax something that can be read in from python for scalars
 			if (int2.eq.2) then
...
!Make axion fraction something that can be read in from python
          		if (int6.eq.6) then
					P%axfrac = axfrac
				end if
...
!Make axion mass something that can be read in from python       
       if (int5.eq.5) then
			P%ma = m_ax
		end if
...
!Make kmax something that can be read in from python        
        if (int4.eq.4) then
			P%transfer%kmax =  transfer_kmax
		end if
        P%transfer%k_per_logint  =  Ini_Read_Int('transfer_k_per_logint')
!Make k density something that can be read in from python
		if (int3.eq.3) then
